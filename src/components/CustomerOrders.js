...
const ORDERS = gql`
    query ($customer_id: uuid!, $limit: Int!, $offset: Int!) {
        orders_aggregate (where: {active: {_eq: true}, customer_id: {_eq: $customer_id}}) {
            aggregate {
                totalCount: count
            }
        }
        orders (where: {active: {_eq: true}, customer_id: {_eq: $customer_id}}, order_by: {date_pickup: desc, id: asc}, limit: $limit, offset: $offset) { 
            id, description, total, date_pickup, date_back
        }
    }
`;

export default function Orders ({ customer_id }) {
    ...
    useEffect(
        () => {
            getOrders({ variables: { customer_id, limit, offset: (getPage - 1) * limit } });
        },
        [customer_id, getPage, getOrders]
    )
